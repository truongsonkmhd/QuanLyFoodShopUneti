 ...
 * @author 84907
 */
public class SizeMatHangDialog extends JDialog implements MouseListener {
    ...
    SizeSpBUS msBUS = new SizeSpBUS();
    ArrayList<SizeSanPhamDTO> list = msBUS.getAll();
    QuanLyThuocTinhSP qltt;
    private final NhomQuyenBUS nhomquyenBus = new NhomQuyenBUS();

    public SizeMatHangDialog(JFrame owner, QuanLyThuocTinhSP qltt, String title, boolean modal, int nhomquyen) {
    ...
    public void initComponent(QuanLyThuocTinhSP qltt) {
        ...
        headTite = new HeaderTitle("Size San Pham");
        ...
        ms = new InputForm("Tên Size mặt hàng");
        ...
        String[] header = new String[]{"Mã Size mặt hàng", "Tên Size mặt hàng"};
    ...
    public void loadDataTable(ArrayList<SizeSanPhamDTO> result) {
        tblModel.setRowCount(0);
        for (SizeSanPhamDTO ncc : result) {
            tblModel.addRow(new Object[]{
                ncc.getIdsize(), ncc.getNamesize()
            });
        }
    }
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == add) {
            if (Validation.isEmpty(ms.getText())) {
                ...
                String tenSize = ms.getText();
                if (msBUS.checkDup(tenSize)) {
                    int id = SizeSanPhamDAO.getInstance().getAutoIncrement();
                    msBUS.add(new SizeSanPhamDTO(id, tenSize));
                    loadDataTable(list);
                    ms.setText("");
                } else {
            ...
            if (index != -1) {
                if (Validation.isEmpty(ms.getText())) {
                    ...
                    if (msBUS.checkDup(tenmau)) {
                        msBUS.update(new SizeSanPhamDTO(list.get(index).getIdsize(), tenmau));
                        loadDataTable(list);
                        ms.setText("");
                    } else {
                        JOptionPane.showMessageDialog(this, "Hệ điều hành đã tồn tại !");
            ...
            ms.setText(list.get(index).getNamesize());
        }
    }
